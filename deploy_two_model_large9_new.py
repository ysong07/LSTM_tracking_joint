...
from G_model_glob_refine import *
...
#    saver_local.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large0/model.ckpt-72001") 
    ...
    with tf.variable_scope('trainable_params_glob') as scope:
        ...
        def feature_extract_mask(batch_frames):
          ...
          def conv_layer(bottom, name,shape):
            with tf.variable_scope(name):
              ...
              return relu
	  def conv_layer_tanh(bottom, name,shape):
            with tf.variable_scope(name):
              filt = get_conv_filter(name,shape)
          ...
          conv4_2 = conv_layer_tanh(conv4_1, "conv4_2",[3,3,64,64])
      ...
      with tf.variable_scope('initial_0'):
        ...
        glob_c_0 = tf.tanh(tf.nn.conv2d(glob_feature_0,c_matrix_0,strides=[1,1,1,1], padding='SAME') + c_bias_0)
      ...
      with tf.variable_scope('initial_1'):
        ...
        glob_c_1 = tf.tanh(tf.nn.conv2d(glob_feature_0,c_matrix_1,strides=[1,1,1,1], padding='SAME') + c_bias_1)
      ...
      G_model_G = G_model_glob(scope="G_model",height=14,width=14,length=FLAGS.length-1,batch_size = FLAGS.batch_size,layer_num_lstm=2,kernel_size=[3,3],kernel_num=[64,64],initial_h_0=glob_h_0,initial_c_0=glob_c_0,initial_h_1=glob_h_1,initial_c_1=glob_c_1,input_features=frame_features,kernel_size_dec=kernel_size_dec,num_dec_input=[64,32,16,8],num_dec_output=[32,16,8,1],layer_num_cnn =4)
...
#    saver.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_resnet_globe/checkpoints/iter_2/test_new_no_smooth_share_feature_weight_0_focal/model.ckpt-9002")
    saver.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_resnet_globe/checkpoints/iter_0/test_0_deconv/model.ckpt-14001")
    #saver.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_1/test_refine_global_0/model.ckpt-16002")
    ...
    for step in xrange(200):
      t= time.time()
      if step ==0:
        masks_resize = cv2.resize(mask[0,:,:,0],(224,224))
	temp = masks_resize
	temp = np.expand_dims(temp,axis=0)
        masks_resize = np.expand_dims(temp,axis=3)
	...
	G_glob_dict = {rest_feature:masks_resize, first_feature:masks_resize}
	#glob_frame,glob_cell,glob_crop_size,glob_center = sess.run([G_model_G.logits,G_model_G.cell_state,G_model_G.crop_size,G_model_G.center],feed_dict= G_glob_dict)
        glob_frame,glob_cell = sess.run([G_model_G.logits,G_model_G.cell_state],feed_dict= G_glob_dict)
...
#        feature_no_mask = sess.run(output_feature_0,feed_dict=feed_dict)
	...
	masks_resize = np.zeros([1,224,224,1])
        temp = cv2.resize(g_predict,(224,224))
        temp = np.expand_dims(temp,axis=0)
	...
	data_handler.set_id(step+1)
        image, mask = data_handler.Get_all()
	G_glob_dict = {rest_feature:masks_resize,glob_c_0:glob_cell[0:1,0,:,:,0:64],glob_h_0:glob_cell[0:1,0,:,:,64:128],glob_c_1:glob_cell[1:2,0,:,:,0:64],glob_h_1:glob_cell[1:2,0,:,:,64:128]}
	glob_frame,glob_cell = sess.run([G_model_G.logits,G_model_G.cell_state],feed_dict= G_glob_dict)
#	glob_frame,glob_cell,glob_crop_size,glob_center = sess.run([G_model_G.logits,G_model_G.cell_state,G_model_G.crop_size,G_model_G.center],feed_dict= G_glob_dict)
...
#        crop_size = int(image.shape[1]/glob_crop_size/224.0*64)*2  
	crop_size = 6*np.int(np.sqrt((mask[0,:,:,0]>0.4).sum())) 
        crop_size = int((crop_size*0.05+old_crop_size*0.95))
        crop_size = min(crop_size,224*3)
        crop_size = max(crop_size,112)
...
#        center = get_center(g_predict.squeeze())
        center = get_center(cv2.resize(glob_frame.squeeze(),(H,W)))
