...
#    output_feature_1 = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,14,14,512])
    ...
    h_0  = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,14,14,512])
    c_0  = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,14,14,512])
    h_1  = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,14,14,512])
    c_1  = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,14,14,512])
    ...
    with tf.variable_scope('trainable_params_local') as scope:
        ...
        with tf.variable_scope('initial_0'):
          ...
          c_0_0 = tf.tanh(c_0_0)
          c_matrix_0_1 = tf.get_variable("matrix_c_1", shape = [3,3,512,512], initializer = tf.random_uniform_initializer(-0.01, 0.01))
          c_bias_0_1 = tf.get_variable("bias_c_1",shape = [512],initializer=tf.constant_initializer(0.01))
          ...
          h_matrix_0_1 = tf.get_variable("matrix_h_1", shape = [3,3,512,512], initializer = tf.random_uniform_initializer(-0.01, 0.01))
          h_bias_0_1 = tf.get_variable("bias_h_1",shape = [512],initializer=tf.constant_initializer(0.01))
        ...
        with tf.variable_scope('initial_1'):
          ...
          c_1_0 = tf.tanh(c_1_0)
          c_matrix_1_1 = tf.get_variable("matrix_c_1", shape = [3,3,512,512], initializer = tf.random_uniform_initializer(-0.01, 0.01))
          c_bias_1_1 = tf.get_variable("bias_c_1",shape = [512],initializer=tf.constant_initializer(0.01))
          ...
          h_matrix_1_1 = tf.get_variable("matrix_h_1", shape = [3,3,512,512], initializer = tf.random_uniform_initializer(-0.01, 0.01))
          h_bias_1_1 = tf.get_variable("bias_h_1",shape = [512],initializer=tf.constant_initializer(0.01))
          h_1 = tf.tanh(tf.nn.conv2d(h_1_0, h_matrix_1_1,strides=[1,1,1,1], padding='SAME') + h_bias_1_1)

        G_model = G_model_(scope="G_model",height=14,width=14,batch_size = FLAGS.batch_size,layer_num_lstm=2,kernel_size=[3,3],kernel_num=[512,512],kernel_size_dec=kernel_size_dec,num_dec_input=[512,256,128,64],num_dec_output=[256,128,64,1],layer_num_cnn =4,initial_h_0=h_0,initial_c_0=c_0,initial_h_1=h_1,initial_c_1=c_1,img_height=W,img_width=H) 
...
#    saver_local.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_3/test_refine_local_0/model.ckpt-8000")
    ...
    saver_local.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_4/test_refine_total_larger0/model.ckpt-16000")
    ...
    saver.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_resnet_globe/checkpoints/iter_2/test_new_no_smooth_share_feature_weight_0/model.ckpt-27000")
...
#	center = np.int32(center).tolist()	
	...
	input_image = np.stack([image],axis=0)
        ...
        feed_dict ={c_0:g_states[0,:,:,:,:,0:512].reshape(FLAGS.batch_size,14,14,512),h_0:g_states[0,:,:,:,:,512:1024].reshape(FLAGS.batch_size,14,14,512),c_1:g_states[1,:,:,:,:,0:512].reshape(FLAGS.batch_size,14,14,512),h_1:g_states[1,:,:,:,:,512:1024].reshape(FLAGS.batch_size,14,14,512),G_model.center:center[::-1],G_model.crop_size:np.asarray(crop_size).reshape(1),G_model.input_frame:image}
        g_predict,g_templates,g_states,g_original_template = sess.run([G_model.predicts,G_model.templates,G_model.cell_states,G_model.original_template],feed_dict= feed_dict)
