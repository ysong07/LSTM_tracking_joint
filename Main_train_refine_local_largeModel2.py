...
from params_refine_local_largeModel2 import * 
...
#      tf.gfile.MakeDirs(FLAGS.train_dir) 
	...
	loss = class_loss + tf.minimum(total_loss,1000) *0.0000
	...
	saver_local = tf.train.Saver(variable_collection_local)
        ...
        saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large2_test/model_keep.ckpt-24")
...
#	saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_more_contaminate_large2/model.ckpt-24000")	
	...
	for step in range(6):
	  ...
	  try:
	    ...
	    file_C = h5py.File('/scratch/ys1297/LSTM_tracking/data_local_vgg_refine_1/'+file_list[contaminate_id][0:-1],'r')
...
#	    features = sess1.run(features_tf,feed_dict =G_feed_dict)
            g_predict= sess1.run([predict_img], feed_dict= G_feed_dict)
	    scipy_io.savemat('test_'+str(file_id)+'.mat',{'features':features,'feature_no_mask':feature_no_mask,'feature_mask':feature_mask,'image':image_temp})
