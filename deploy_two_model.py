...
#    saver_local.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_1/test_refine_shuffle_0/model.ckpt-40000")	
    saver_local.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_2/test_refine_total_0/model.ckpt-24000")
...
#    saver.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_1/test_refine_global_0/model.ckpt-1000")
	...
	#first_template = cv2.getRectSubPix(image[0,:,:,:],(int(crop_size),int(crop_size)),(center[0],center[1])) 
    	#first_template = cv2.resize(first_template,(224,224)).reshape(FLAGS.batch_size,224,224,3)
	
	#first_mask = cv2.getRectSubPix(mask[0,:,:,0:1],(int(crop_size),int(crop_size)),(center[0],center[1]))>0.0
	#first_mask = cv2.resize(first_mask,(224,224)).reshape(224,224,1)
	...
	save_file_name = './img/'+re.sub('origin.h5','',test_file_name)+'_'+str(step)+'.mat'
	...
	ratio = np.array([float(mask.shape[1])/mask.shape[2]]).reshape(1,1)
#	G_glob_dict = {rest_feature:cv2.resize(mask[:,:,:,0].squeeze(),(224,224)).reshape(1,224,224,1), G_model_G.ratio:ratio,glob_c_0:glob_cell[0:1,0,:,:,0:64],glob_h_0:glob_cell[0:1,0,:,:,64:128],glob_c_1:glob_cell[1:2,0,:,:,0:64],glob_h_1:glob_cell[1:2,0,:,:,64:128]}
