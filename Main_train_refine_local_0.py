...
#      tf.gfile.MakeDirs(FLAGS.train_dir) 
	...
	def listen(num,q):
	  for i in range(100):
	    q.put( comm.recv(source=0,tag=1))
	def fetch(num,q):
          with tf.Graph().as_default():
            kernel_size_dec = []
            kernel_size_dec.append([5,5])
            ...
            feature = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,224,224,3])
            mask_tensor = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,224,224,64])
        
            output_feature_0 = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,14,14,512]) ## all feature 
        ...
        #    output_feature_1 = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,14,14,512]) 
	      ...
	      loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels= ground_truth, logits= G_model.logits))
              ...
              variable_collection = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,"trainable_params_local")
	    ...
	    #saver.restore(sess, "/scratch/ys1297/LSTM_tracking/source_cross_vgg_local/checkpoints/local_0/cross0/model.ckpt-86000")
	    ...
	    for step in range(100000):
	      ...
	      if data is not None:
	        t= time.time()	
	        print "batch id : "+ str(data['id'])
		feature_0_mask = data['feature_mask']
	        feature_0_no_mask = data['feature_no_mask']
	        all_features = data['all_features']
	        masks = np.stack([data['masks']],axis =0)
	        all_images = data['image_array']
	...
	q  = Queue.Queue(maxsize=5)
        thread1 = threading.Thread(target = listen,args =(1,q))    
	thread2 = threading.Thread(target = fetch,args= (1,q))
	time.sleep(10)
	thread1.start()
	thread2.start()		
