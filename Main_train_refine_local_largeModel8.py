...
from params_refine_local_largeModel8 import * 
...
def calculate_IOU(GT, predict,thre=0.4):
...
def evaluate_acc(masks,predict):
    acc = np.zeros(31)
    predict = predict.squeeze()
    masks = masks.squeeze()
    threshold=0.3
    masks[masks>threshold]=True
    masks[masks<=threshold]=False
    predict[predict>threshold]=True
    predict[predict<=threshold]=False
    for i in range(31):
        acc[i]=np.mean(predict[i+1,:,:]==masks[i+1,:,:])
    return acc
...
#      tf.gfile.MakeDirs(FLAGS.train_dir) 
	...
	loss = class_loss + tf.minimum(total_loss,1000) *0.000
...
#        saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large8/model_keep.ckpt-24")
#	saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_2/model.ckpt-34000")
	saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large8_no_contaminate/model_keep.ckpt-20000")	
	temp_file = open(FLAGS.test_file_dir,'r')
	file_list = temp_file.readlines()
	file_id = 0
	np.random.seed(0)
	acc = np.zeros(31)
	mask_stats=0
        IOU_list = np.zeros(32)
        for step in range(100):
	  print file_list[file_id][0:-1]
	  try:
            file = h5py.File('/scratch/ys1297/LSTM_tracking/data_local_vgg_refine_7/'+file_list[file_id][0:-1],'r')	
	    ...
	    file_C = h5py.File('/scratch/ys1297/LSTM_tracking/data_local_vgg_refine_7/'+file_list[contaminate_id][0:-1],'r')
	    ...
	    for image_id in range(32):
	      #if np.random.random(1)> (50000.0-float(step))/40000.0:
              if np.random.random(1)> 1.1 and image_id !=0:
		rand_size = np.random.rand(1)*0.1+0.9
                shift = np.random.randint(-10,10,size=2) 
                M = np.float32([[rand_size[0],0,shift[0]],[0,rand_size[0],shift[1]]])
#                M = np.float32([[1,0,0],[0,1,0]])
                image_temp[image_id,:,:,:] = cv2.warpAffine(image_C[:,image_id,:,:,:].squeeze(),M,(224,224))
...
#	    G_feed_dict = {output_feature_0: feature_no_mask, output_feature_1: feature_mask, ground_truth:masks,G_model.input_features:feature}
 	    G_feed_dict = {output_feature_0: feature_no_mask, output_feature_1: feature_mask, ground_truth:masks,batch_frames:image_temp}
            g_predict,g_states= sess1.run([predict_img,G_model.lstm_state], feed_dict= G_feed_dict)
#	    mask_stats+= masks.mean()
	#    acc += evaluate_acc(g_predict[0],masks)
	#    print acc
#	    scipy_io.savemat('test_'+str(file_id)+'.mat',{'image':image_temp,'masks':masks_temp,'predict':g_predict,'g_state_1':g_states[1,:,:,:,:,0:256],'g_state_0':g_states[0,:,:,:,:,0:256],'h_state_1':g_states[1,:,:,:,:,256:512],'h_state_0':g_states[0,:,:,:,:,256:512]})
           #scipy_io.savemat('test.mat',{'image':image_temp,'masks':masks_temp,'predict':g_predict,'feature':feature,'feature_no_mask':feature_no_mask,'feature_mask':feature_mask})
#	    g_summary,_,g_loss = sess1.run([summary,train_op,loss],feed_dict= G_feed_dict)
#	    summary_writer.add_summary(g_summary, step)
	    masks[masks>0.5]=255
            masks[masks<=0.5]=0
	    masks = masks.squeeze()
            predict = g_predict.squeeze() 
	    for k in range(32):
                IOU,no_iter = calculate_IOU(masks[k,:,:].squeeze(),predict[k,:,:].squeeze())
                IOU_list[k]+=IOU
	    elapsed = time.time() - t
          except:
	    elapsed = 0
	  ...
	  if file_id >= len(file_list):
	    file_id = 0
	print IOU_list/100.0
	scipy_io.savemat('./test_8.mat',{'IOU_list':IOU_list}) 
        #acc = acc/600
        #mask_stats=mask_stats/600
        #scipy_io.savemat('./test_9.mat',{'acc':acc,'mask_stats':mask_stats})
#          if step %2000==0:
#            checkpoint_path = os.path.join(FLAGS.train_dir, 'model_keep.ckpt')
