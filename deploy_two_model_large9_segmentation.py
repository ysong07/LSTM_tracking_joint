...
#    saver_local = tf.train.import_meta_graph("/scratch/ys1297/LSTM_tracking/source_segmenation/checkpoints_5/test_refine_total_large7_segmentation/model_keep.ckpt-60.meta")
#    saver_local.restore(sess,"/scratch/ys1297/LSTM_tracking/source_segmenation/checkpoints_5/test_refine_total_large9_segmentation/model_keep.ckpt-60") 
#####  
#    variable_collection_local = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,"segmentation")
...
#    np.save('feature_mapping9_segmentation.npy',dict_params)
#    pdb.set_trace()
###
    kernel_size_dec = []
...
#    output_feature_1 = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,14,14,512])
    ...
    with tf.variable_scope('segmentation') as scope:
        ...
        G_model = G_model_(scope="G_model",height=14,width=14,batch_size = FLAGS.batch_size,layer_num_lstm=2,kernel_size=[3,3],kernel_num=[256,256],kernel_size_dec=kernel_size_dec,num_dec_input=[512,256,128,64],num_dec_output=[256,128,64,1],layer_num_cnn =4,initial_h_0=h_0,initial_c_0=None,initial_h_1=None,initial_c_1=None,img_height=W,img_width=H) 
...
#        feature_no_mask = sess.run(output_feature_0,feed_dict=feed_dict)
        ...
        feed_dict ={G_model.input_frame:image,G_model.center:center[::-1],G_model.crop_size:np.asarray(crop_size).reshape(1)}
        g_predict,g_templates,g_original_template = sess.run([G_model.predicts,G_model.templates,G_model.original_template],feed_dict= feed_dict)
...
#	center = np.int32(center).tolist()	
	...
	scipy_io.savemat(save_file_name,{'glob_frame':glob_frame,'image':image,'mask':mask,'predict':g_predict,'g_templates':g_original_template,'g_template_predict':g_templates})
        ...
        masks_resize = cv2.resize(mask[0,:,:,0],(224,224)).reshape(1,224,224,1)/255.0
#       masks_resize = np.zeros([1,224,224,1])
        #temp = cv2.resize(g_predict,(224,224)).reshape(1,224,224,1)
        #masks_resize = temp*255.0
...
#        crop_size = 4*np.int(np.sqrt((g_predict.squeeze()>0.4).sum()))
        ...
        center = ((old_center*0.6+ np.asarray(center)*0.4)).tolist()
        input_image = np.stack([image],axis=0)
