...
def main(argv=None):  # pylint: disable=unused-argument
	...
	saver_local = tf.train.Saver(variable_collection_local)
        saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large8/model.ckpt-8000")
...
#	    feature = np.stack([file['feature'][id]])
	    ...
	    sub_id_seq =np.arange(32)
	    np.random.shuffle(sub_id_seq)
	    for sub_step in range(32):
	      sub_id = sub_id_seq[sub_step]
              blobs = {} 
	      blobs['input_fea'] = feature[sub_id:sub_id+1,:,:,:]
	      blobs['gt_boxes'] = np.stack([bbx[sub_id:sub_id+1,:]])
