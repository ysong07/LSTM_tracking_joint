...
from params_refine_bbx7 import * 
...
import sys
sys.path.insert(0,'/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/anchor_gen_bbox_reg-2_large')
...
def main(argv=None):  # pylint: disable=unused-argument
	...
	with tf.variable_scope('bbx_params') as scope:
	  LSTM_feature_np = tf.placeholder(tf.float32, shape=[32,14,14,256])

          c_matrix_0_0 = tf.get_variable("matrix_conv5", shape = [3,3,256,512], initializer = tf.random_uniform_initializer(-0.01, 0.01))
          c_bias_0_0 = tf.get_variable("bias_conv5",shape = [512],initializer=tf.constant_initializer(0.01))
          conv5_lstm = tf.nn.conv2d(LSTM_feature_np,c_matrix_0_0,strides=[1,2,2,1], padding='SAME') + c_bias_0_0	  
	
	  network = C3DTrackNet(batch_size=32)
	...
	saver_local = tf.train.Saver(variable_collection_local)
        saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large7/model.ckpt-8000")
...
#	    feature = np.stack([file['feature'][id]])
	    ...
	    tmp =copy.copy(bbx)
	    bbx[:,1]= tmp[:,0] - tmp[:,2]/2.0
	    bbx[:,0]= tmp[:,1] - tmp[:,3]/2.0
	    bbx[:,3]= tmp[:,0] + tmp[:,2]/2.0
            bbx[:,2]= tmp[:,1] + tmp[:,3]/2.0

	    bbx[bbx <=0.0] = 0.001
	    bbx[bbx>=1.0] = 0.999
	    bbx = bbx*224.0
	    bbx = np.concatenate((bbx,np.ones((32,1))),axis=1)
            blobs = {} 
	    blobs['input_fea'] = feature
	    blobs['gt_boxes'] = np.stack([bbx])
	    blobs['im_info'] =np.repeat(np.array([[224.,224.,1.0]]),32,axis=0)
	    feed_dict = {LSTM_feature_np:feature,network._gt_boxes:np.stack([bbx]),network._im_info:np.repeat(np.array([[224.,224.,1.0]]),32,axis=0)}
	    rpn_bbox_pred, rpn_bbox_targets, total_loss , _ = sess1.run([network._losses["rpn_cross_entropy"],network._losses['rpn_loss_box'],network._losses['total_loss'],train_op],feed_dict=feed_dict)
            print  rpn_bbox_pred, rpn_bbox_targets,total_loss
	
	    #rpn_loss_cls, rpn_loss_box, total_loss, bbx_predict = network.train_step(sess1, blobs, train_op)
