...
from params_refine_local_3 import *
from G_model_glob_share_weight import *
from G_inference_new3 import *
import os 
import time
import sys
...
sys.path.insert(0,'/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/anchor_gen_bbox_reg-5')
from nets.c3d_tracknet import C3DTrackNet
from model.config import cfg
from model.bbox_transform_btube import bbox_transform_inv,clip_boxes
import os
import glob as glob
import pdb
import copy
...
def _get_predicted_tubes(roi_tubes,box_deltas):
    ...
    predicted_tubes = np.zeros((roi_tubes.shape[0],roi_tubes.shape[1],
                                    4*2))
    for ii in range(cfg.NUM_FRAMES_PER_CLIP):
        if cfg.useNormalizedTrans:
            ...
            for kk in range(2):
                predicted_tubes[:,ii,kk*4:(kk+1)*4] = pred_boxes[:,kk*4:(kk+1)*4] #1st background, then class 1
    return predicted_tubes


def tube_detect(sess, net, blobs):
    proposals_tube,bbox_pred,scores= net.test_image(sess, blobs['data'], blobs['im_info'])
    ...
    proposals_tube = np.squeeze(proposals_tube) #300*4
    pred_tubes = np.squeeze(pred_tubes)#300*8 
    return proposals_tube, pred_tubes, scores
...
def train(test_file_name):
  ...
  with tf.Graph().as_default():
    ...
    with tf.variable_scope('bbx_params') as scope:
      ...
      network.create_architecture(sess, 'TEST', 2,
                  tag='default', anchor_scales=cfg.ANCHOR_SCALES)


    saver_bbx = tf.train.Saver()
   ...
   # saver_bbx = tf.train.Saver(variable_collection_bbx)
    saver_bbx.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_bbx3/model.ckpt-20000")

#    """ storing feature mapping """
#    saver_local = tf.train.import_meta_graph("/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large3/model.ckpt-72001.meta") 
#    saver_local.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large3/model_keep.ckpt-24")
...
#    np.save('feature_mapping3.npy',dict_params)
#    pdb.set_trace()
###
    kernel_size_dec = []
...
#    saver_local.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_contaminate_large2/model.ckpt-16000")
    ...
    saver.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_resnet_globe/checkpoints/iter_2/test_new_no_smooth_share_feature_weight_3/model.ckpt-27000")
#    saver.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_1/test_refine_global_0/model.ckpt-1000")
    ...
    try: 
      os.mkdir('./bbx/'+re.sub('origin.h5','',test_file_name))
    except:
      pass
...
#        feature_no_mask = sess.run(output_feature_0,feed_dict=feed_dict)
	...
	g_predict,g_templates,g_states,g_original_template,LSTM_feature= sess.run([G_model.predicts,G_model.templates,G_model.cell_states,G_model.original_template,G_model.lstm_output],feed_dict= feed_dict)
        blobs = {}
        blobs['data'] = LSTM_feature[0,:,:,:]
        blobs['im_info'] = np.array([[224.,224.,1.]])
	proposals_tube, pred_tubes, scores = tube_detect(sess, network, blobs)
#        proposals_tube,proposals_tube_scores,rois = network.test_image(sess, blobs['input_fea'], blobs['im_info'])
        temp = pred_tubes[0,:]
        x = (temp[0]+temp[2])/2.0/224
        y = (temp[1]+temp[3])/2.0/224
        w = (temp[2]-temp[0])/224.0
        h = (temp[3]-temp[1])/224.0
        bbx_predict = np.asarray([x,y,w,h])
        save_file_name = './bbx/'+re.sub('origin.h5','',test_file_name)+'/'+str(step)+'.mat'
	...
	crop_size = int((crop_size*0.1+old_crop_size*0.9))
	crop_size = min(crop_size,224*3)
        crop_size = max(crop_size,112)
 	center = (np.asarray(center_0)*(6.0-step)+np.asarray(center_1)*float(step))/6.0
	center = ((old_center*0.9+ center*0.1)).tolist()
