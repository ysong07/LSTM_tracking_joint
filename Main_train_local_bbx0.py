...
from params_refine_bbx0 import * 
...
#      tf.gfile.MakeDirs(FLAGS.train_dir) 
	...
	""" building training graph """
        ...
        with tf.variable_scope('bbx_params') as scope:
          gt_bbx = tf.placeholder(tf.float32,shape = [32,4])
          input_ = G_model.lstm_output[0,:,:,:,:]
        ...
        loss = tf.reduce_mean(tf.nn.l2_loss(fc2-gt_bbx))
...
#           feature = np.stack([file['feature'][id]])
	    ...
	    file.close()
            ...
            t= time.time()
            G_feed_dict = {output_feature_0: feature_no_mask, output_feature_1: feature_mask,G_model.input_features:feature,gt_bbx:bbx}
