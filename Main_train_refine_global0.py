...
from params_refine_global_0 import * 
  ...
  with tf.Graph().as_default():
	...
	f_scope.reuse_variables()
      ...
      with tf.variable_scope('initial_0'):
        ...
        c_0 = tf.tanh(tf.nn.conv2d(output_feature_0,c_matrix_0,strides=[1,1,1,1], padding='SAME') + c_bias_0)
      ...
      with tf.variable_scope('initial_1'):
        ...
        c_1 = tf.tanh(tf.nn.conv2d(output_feature_0,c_matrix_1,strides=[1,1,1,1], padding='SAME') + c_bias_1)
...
#    saver_load.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_resnet_globe/checkpoints/iter_2/test_new_no_smooth_share_feature_weight_0/model.ckpt-27000")
    saver_load.restore(sess,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_1/test_refine_global_0/model.ckpt-16002")
...
#      print("time per batch is " + str(elapsed)," loss : "+str(g_loss))
      ...
      center_3 =[]
      for i in range(30):
	center_1.append(get_center(temp_mask[0,i+1,:,:,:].squeeze()))
        center_2.append(get_center(predict_mask[0,i,:,:,:].squeeze()))
	center_3.append(get_center(g_predict[0,i,:,:,:].squeeze()))
      error1=((np.asarray(center_1)-np.asarray(center_2))*(np.asarray(center_1)-np.asarray(center_2))).sum()
      error2=((np.asarray(center_1)-np.asarray(center_3))*(np.asarray(center_1)-np.asarray(center_3))
).sum()
      print g_predict.sum()/31/224.0/224.9 
      pdb.set_trace()	
      print error1-error2	
