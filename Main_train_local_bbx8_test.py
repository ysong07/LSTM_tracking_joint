...
from params_refine_bbx8 import * 
...
import sys
sys.path.insert(0,'/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/anchor_gen_bbox_reg-2')
...
#      tf.gfile.MakeDirs(FLAGS.train_dir) 
      ...
      with tf.Graph().as_default():
        ...
        with tf.variable_scope('bbx_params') as scope:
          ...
          network.create_architecture(sess1, 'TEST', 2,
        ...
        saver_bbx.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_4/test_refine_bbx8/model.ckpt-100")
	...
	saver_local = tf.train.Saver(variable_collection_local)
        saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large8/model.ckpt-16000")
#	saver_bbx.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_4/test_refine_bbx8/model.ckpt-1000")
...
#	    feature = np.stack([file['feature'][id]])
	    ...
	    scipy_io.savemat('test.mat',{'bbx':bbx,'image':image})
	    t= time.time()
	    ...
	    pdb.set_trace()
            blobs = {} 
	    blobs['input_fea'] = feature[0:1,:,:,:]
	    #blobs['gt_boxes'] = np.asarray([bbx[i].tolist()+[1.0]]).reshape(1,1,5)
#	    blobs['im_info'] =np.repeat(np.array([[224.,224.,1.0]]),32,axis=0)
	    blobs['im_info'] = np.array([[224.,224.,1.0]])
	    proposals_tube,proposals_tube_scores,rois = network.test_image(sess1, blobs['input_fea'], blobs['im_info'])
