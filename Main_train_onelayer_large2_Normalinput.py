...
from params_onelayer_normalinput import * 
from G_model_onelayer import *
import os 
import time
...
#      tf.gfile.MakeDirs(FLAGS.train_dir) 
	...
	batch_frames= tf.placeholder(tf.float32,shape=[16,224,224,3])
	...
	""" building training graph """
        ...
        ground_truth = tf.placeholder(tf.float32,shape=[FLAGS.batch_size,16,224,224,1])
	  ...
	  with tf.variable_scope('feature_mapping'):
	    ...
	    f_0 = tf.sigmoid(f_0)
          ...
          with tf.variable_scope('initial_0'):
            ...
            h_0 = tf.sigmoid(tf.nn.conv2d(h_0_0, h_matrix_0_1,strides=[1,1,1,1], padding='SAME') + h_bias_0_1)
 	    h_0 = h_0*tf.nn.tanh(c_0) 
  
          G_model = G_model_local_refine(scope="G_model",height=14,width=14,length=16,batch_size = FLAGS.batch_size,layer_num_lstm=1,kernel_size=[3],kernel_num=[256],kernel_size_dec=kernel_size_dec,num_dec_input=[256,128,64,32],num_dec_output=[128,64,32,1],layer_num_cnn =4,initial_h_0=h_0,initial_c_0=c_0,input_feature=tf.stack([f_0]))
	#  G_model.input_features = tf.stack([f_0])
        ...
        predict_img = tf.reshape(tf.sigmoid(G_model.logits),[16,224,224,1])
	total_loss = tf.reduce_mean(tf.image.total_variation(predict_img))
	loss = class_loss + tf.minimum(total_loss,1000) *0.0
	...
	saver_local = tf.train.Saver(variable_collection_local)
        saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/onelayer_normalinput/model.ckpt-8000")
