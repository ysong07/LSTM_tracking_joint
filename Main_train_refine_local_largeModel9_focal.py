...
from params_refine_local_largeModel9 import * 
...
#      tf.gfile.MakeDirs(FLAGS.train_dir) 
	...
	""" building training graph """
        ...
        inverse_predict = 1-tf.sigmoid(G_model.logits)
        preds = tf.where(tf.equal(ground_truth,1),inverse_predict,1-inverse_predict)
        class_loss = tf.reduce_mean(-1.0*(1-preds)**2*tf.log(preds+1e-7))
	class_loss = tf.reduce_mean(-1.0*(preds)**2*tf.log(preds+1e-7))
...
#	saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large9_focal/model.ckpt-2000")
#        saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large9/model_keep.ckpt-24")
#	saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_2/model.ckpt-34000")
#	saver_local.restore(sess1,"/scratch/ys1297/LSTM_tracking/source_cross_vgg_refine/checkpoints_5/test_refine_total_large9_nocontaminate/model.ckpt-20000")	
	...
	for step in range(20001):
	  ...
	  try:
            file = h5py.File('/scratch/ys1297/LSTM_tracking/data_local_vgg_refine_9/'+file_list[file_id][0:-1],'r')	
	    ...
	    file_C = h5py.File('/scratch/ys1297/LSTM_tracking/data_local_vgg_refine_9/'+file_list[contaminate_id][0:-1],'r')
	    ...
	    for image_id in range(32):
              ...
              if np.random.random(1)> 0.7 and image_id !=0:
...
#	    G_feed_dict = {output_feature_0: feature_no_mask, output_feature_1: feature_mask, ground_truth:masks,G_model.input_features:feature}
       ...
       #     g_predict= sess1.run([predict_img], feed_dict= G_feed_dict)
